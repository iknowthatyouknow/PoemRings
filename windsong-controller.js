...
(function () {
  ...
  // Try to add an item “WindSong” under an existing nav; fallback to floating activator
  ...
  function buildPanel(initVals, onApplyCb, onTriggerCb, onExitCb) {
    ...
    el.innerHTML = `
      ...
      <div class="ws-help">
        Breath = seconds between drifting lines. Elegra = reveal pacing (seconds). Rez = times per hour (1 = once per refresh).
      </div>
    ...
    el.querySelector('#ws-trigger').addEventListener('click', () => {
      onTriggerCb(); // runs a Wind’s Song cycle immediately (useful for testing)
    });
  ...
  function onTriggerNow() {
    // Immediately run a drift/reveal cycle (doesn’t change timers)
  ...
  function svgBreath() {
    // "poem ↔ poem" glyph: simple two blocks with a bidirectional arrow
    return `
